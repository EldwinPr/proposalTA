...
\begin{enumerate}
    \item Definisi Beban Kerja (Workload) Spesifik ERP \cite{maddodi2018generating}
    \item Metrik Performa Aplikasi (Throughput \& Response Time) \cite{iso25010}
    \item Metrik Utilisasi Sumber Daya (CPU, Memori, I/O) \cite{blem2013power}
    \item Analisis Stabilitas dan Tingkat Kesalahan (Stability \& Error Rate)
    \item Metode Pengujian Beban dan Stres (Load \& Stress Testing) \cite{maddodi2018generating}
    \item Metodologi Benchmarking Komparatif (SBC vs. Cloud)
\end{enumerate}

\subsection{Evaluasi Finansial}

\begin{enumerate}
    \item Kerangka Kerja Total Cost of Ownership (TCO) \cite{buijsse2024reference}
    \item Identifikasi Komponen Biaya Cloud/VPS (OPEX) \cite{Younus2024}
    \item Identifikasi Komponen Biaya On-Premise/SBC (CAPEX \& OPEX) \cite{Younus2024}
    \item Model Perhitungan TCO untuk Periode 3 \& 5 Tahun
    \item Analisis Titik Impas (\textit{Break-Even Point}) \cite{buijsse2024reference}
\end{enumerate}
...
\begin{itemize}
    \item Pemenuhan Syarat Sistem Minimum ERP \cite{odoo2025documentation, erpnext2024introduction}
    \item Pencapaian Ambang Batas Performa (Waktu Respons \& Throughput) \cite{iso25010}
    \item Stabilitas dan Keandalan Sistem (Uptime \& Error Rate)
    \item Potensi Skalabilitas (Vertikal/Horizontal) \cite{Putra2019}
    \item Pertimbangan Keamanan Infrastruktur Dasar
\end{itemize}

\subsection{Kelayakan Finansial}

\begin{itemize}
    \item Keunggulan TCO Dibandingkan Alternatif Cloud \cite{buijsse2024reference}
    \item Periode Pengembalian Investasi (Payback Period) yang Wajar
    \item Keterjangkauan Biaya untuk Skala UKM atau Eksperimental
    \item Analisis Nilai Jangka Panjang (\textit{Value for Money})
    \item Perbandingan Biaya dengan Solusi SaaS \textit{Entry-Level}
\end{itemize}

\subsection{Trade-off Analysis Framework}

\begin{itemize}
    \item Model Penilaian Tertimbang (Performa vs. Biaya vs. Kontrol)
    \item Identifikasi \textit{Trade-off} Kritis (misal: Biaya vs. Kemudahan Pemeliharaan)
    \item Analisis Faktor Kualitatif (Kedaulatan Data, Fleksibilitas)
    \item Matriks Keputusan untuk Rekomendasi Platform
\end{itemize}